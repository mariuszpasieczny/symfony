...
class AclAdminExtension extends AdminExtension {
    ...
    SecurityContextInterface $securityContext, Connection $databaseConnection, array $roleHierarchy = array()
    ) {
    ...
    public function configureQuery(AdminInterface $admin, ProxyQueryInterface $query, $context = 'list') {
        ...
        if (
                !$admin->isAclEnabled() || !$this->securityContext->getToken() || $admin->isGranted(sprintf($admin->getSecurityHandler()->getBaseRole($admin), 'ADMIN'))
        ) {
            return;
        }
        ...
        if (!empty($identityIds) && $classId) {
            $entriesStmt = $this->databaseConnection->executeQuery(
                    'SELECT DISTINCT object_identifier FROM acl_entries AS ae JOIN acl_object_identities AS aoi ON ae.object_identity_id = aoi.id WHERE ae.class_id = ? AND ae.security_identity_id IN (?) AND (? = ae.mask & ? OR ? = ae.mask & ? OR ? = ae.mask & ? OR ? = ae.mask & ?)', array(
                $classId,
                $identityIds,
